...
var jobProvider = require("../data-access/job-provider");
//
const TB_TABLE = "tb_job";
const API_SERVICES = "/api/job/";
    ...
    return [
            ...
            jobProvider
              ...
              .then(s => {
                if (s.affectedRows) {
                  ...
                  res.send(
                    responseUtils.build(3, null, "Thêm không thành công")
                  );
                }
              })
              .catch(e => {
      ...
      () => {
        /* 
                ...
                */
        let jsonParser = bodyParser.json();
        app.put(API_SERVICES + "set-my-job", jsonParser, function(req, res) {
          const user = authUtils.getUser(req.headers);
          if (user && user.user && user.user.id) {
            let { jobs } = req.body;
            jobProvider
              .setMyJob(user.user.id, jobs || [])
              .then(s => {
                if (s.affectedRows !== 0) {
                  res.send(responseUtils.build(0, s));
                  return;
