...
class PayMentAgent extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            createdDate: '',
            createdUser: '',
            // status: this.props.userApp.paymentAgent.status ? this.props.userApp.paymentAgent.status : -1,
            status: -1,
            type: -1,
            totalPage: 0,
            listPaymentMethod: []
    ...
    loadPage(item) {
        ...
        paymentAgentProvider.search(params, item ? true : false).then(s => {
    ...
    handleChangeFilter(event, action) {
        ...
        // if (action === 1) {
        //     this.props.dispatch({ type: constants.action.action_payment_agent_status, value: event.target.value })
        //     dataCacheProvider.save("", constants.key.storage.payment_agent_status, event.target.value)
        //     this.setState({
        //         page: 0,
                                            ...
                                            return (
                                                <div key={index} className={item.paymentAgent.status === 1 ? "bank-item" : "bank-item bank-inactive"}>
                                                    <div className="bank-top">
                                                        ...
                                                        <div className="bank-right">
                                                            <a href="#" onClick={() => { this.getDetail(item, "create") }}>
                                                                <img src="/images/edit.png" alt="" />
                                                            </a>
                                                            <p className="bank-type"><span className="bank-active">{item.paymentAgent.status === 1 ? "Đang hoạt động" : "Đã khóa"}</span></p>
                                                        </div>
