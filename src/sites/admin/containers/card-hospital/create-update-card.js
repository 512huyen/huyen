...
class CreateUpdateCard extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            listPaymentAgent: this.props.listPaymentAgent && this.props.listPaymentAgent,
            ...
            hospitalId: this.props.data && this.props.data.cardTransferHistory && this.props.data.cardTransferHistory.hospital && this.props.data.cardTransferHistory.hospital.id ? this.props.data.cardTransferHistory.hospital.id : this.props.userApp.currentUser && this.props.userApp.currentUser.hospital && this.props.userApp.currentUser.hospital.id,
    ...
    render() {
        ...
        const { dataCard, transferUser, paymentAgentId, receiverUser, hospitalId, quantity, transferDate, listHospitalCreate, listPaymentAgent, listPaymentMethodsHospital, listBank, cardNoFrom, cardNoTo } = this.state;
        return (
                ...
                <Dialog
                    ...
                    <ValidatorForm onSubmit={this.create}>
                        <DialogTitle id="alert-dialog-slide-title" className="title-popup">
                            {dataCard.cardTransferHistory && dataCard.cardTransferHistory.id ? 'Sửa sử lịch bàn giao thẻ ' : 'Thêm lịch sử bàn giao thẻ'}
                        ...
                        <DialogContent>
                            <Grid container spacing={16} className="hospital-header-card">
                                ...
                                </Grid>
                                {
                                    dataCard && dataCard.cardTransferHistory && dataCard.cardTransferHistory.id ?
                                        <Grid item xs={12} md={8} className="grid-title-2-disabled">
                                            <div>{moment(dataCard.cardTransferHistory.transferDate).format("DD-MM-YYYY")}</div>
                                        </Grid>
                                                ...
                                                return (
                                                    <Grid item xs={12} md={6} className="hospital-padding" key={index}>
                                                        <Radio
                                                            ...
                                                            onChange={(event) => { this.data2.bankId = event.target.value; this.setState({ bankId: event.target.value }) }}
                                                            value={item.id}
                                ...
                                </Grid>
                                {
                                    dataCard && dataCard.cardTransferHistory && dataCard.cardTransferHistory.id ?
                                        <Grid item xs={12} md={8} className="grid-title-2-disabled">
                                            <div>{dataCard.cardTransferHistory.quantity}</div>
                                        </Grid> :
                                        ...
                                        {/* {
                                            listPaymentAgent && listPaymentAgent.length > 0 ? listPaymentAgent.map((item, index) => {
                                                return (
                                                    <Grid item xs={12} md={6} className="hospital-padding" key={index}>
                                                        <Radio
                                                            checked={this.state.paymentAgentId === item.paymentAgent.id.toString()}
                                                            onChange={(event) => { this.data2.paymentAgentId = event.target.value; this.setState({ paymentAgentId: event.target.value }) }}
                                                            value={item.paymentAgent.id}
                                                            name="radio-button-demo"
                                                            inputProps={{ 'aria-label': item.paymentAgent.id.toString() }}
                                                        /> {item.paymentAgent.nameAbb}
                                                    </Grid>
                                                )
                                            }) : null
                                        } */}
                                ...
                                {/* <Grid item xs={12} md={8} className="news-title-select hospital-title-name">
                                    <SelectValidator
                                        value={hospitalId}
                                        onChange={(event) => {
                                ...
                                <Grid item xs={12} md={8} className="grid-title-2-disabled">
                                    <div>{this.props.userApp.currentUser.hospital && this.props.userApp.currentUser.hospital.name}</div>
                                </Grid>
                            </Grid>
                        </DialogContent>
