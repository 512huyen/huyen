...
class CreateUpdateHospital extends React.Component {
    ...
    create = async () => {
        let { dataHospital, name, fileNames, issueDateTaxNo, code, phone, fax, address, taxNo, logo, status, listPaymentMethod } = this.state;
        let data = {}
        let data3 = []
        ...
        if (fileName.endsWith(".xls") || fileName.endsWith(".xlsx")) {
            imageProvider.uploadFile(event.target.files[0]).then(s => {
                if (s && s.data.code == 0 && s.data.data) {
                    this.setState({
                        fileAccount: s.data.data.file,
                        fileNames: s.data.data.file
                    })
    ...
    render() {
        ...
        const { dataHospital, name, taxNo, code, phone, fax, address, fileAccount, logo, listKeyMethod, listPaymentMethod, checkMethodValidate } = this.state;
        return (
                                                                ...
                                                                listPaymentMethod[item] && listPaymentMethod[item].length > 0 ? listPaymentMethod[item].map((item2, index2) => {
                                                                    return (
                                                                        <Grid item xs={12} md={6} className="hospital-padding hospital-padding-style-1" key={index2}>
                                                                            ...
                                                                            <a className="hospital-name">{item2.nameAbb}</a>
                                                                        </Grid>
                                                                    )
                                                                }) : null
                                                            }
